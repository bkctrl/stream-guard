...
### Variables ###
buffer = BytesIO(b"")
tmp_path = os.path.join(os.path.dirname(os.getcwd()), "tmp\\")
ban_word = "chocolate"
rep_word = "wolf"
data_queue = Queue()
transcription = ['']
...
### Functions ###
def get_latest_name(filepath = os.path.join(tmp_path, "output-wf-")):
...
def handle_client(conn, audio_dir="tmp_path"):
...
def process_audio_data(audio_data, phrase_complete=False):
    ...
    for line in transcription:
        ...
        print(new_line)
        with open(os.path.join(tmp_path, "transcript.txt"), "a", encoding='utf-8') as f:
            f.write(f"{new_line}\n")
            f.close()
    # Flush stdout.
...
def get_transcript():
    with open("tmp/transcript.txt", "r", encoding='utf-8') as f:
        return f.read()
